...
var Uppy = /*#__PURE__*/function () {
  ...
  _proto._addListeners = function _addListeners() {
    ...
    this.on('upload-started', function (filesData) {
      if (!filesData) {
        _this6.setState({
          uploadProcessingStarted: true
        });

        return;
      }

      for (var _iterator = _createForOfIteratorHelperLoose(filesData), _step; !(_step = _iterator()).done;) {
        var fileData = _step.value;
        var file = fileData['file'];
        var upload = fileData['upload'];
    ...
    this.on('start-event-completed', function () {
      _this6.setState({
        uploadInitilizationComplete: true
      });
    });
